            ...
            if self._training_mode == 'skip_adapters_mode':
                ...
                if len(self.task_sizes) == 1:
                    ...
                    for i, adapter in enumerate(getattr(self, adapter_id)):
                        adapter_weight = adapter.state_dict()
                        adapter_weights.append(adapter_weight)
                    average_adapters = self.average_dicts(adapter_weights)
                    prev_adapter_weight_2 = copy.deepcopy(adapter_weights[-1])
                    getattr(self, adapter_id)[-1].load_state_dict(average_adapters)
                    getattr(self, adapter_id)[-1].eval()
                    for i in range(len(self.task_sizes)):
                        if b != self.forward_batch_size:
                            ...
                            adapter_input = input
                        adapter_output = getattr(self, adapter_id)[-1](adapter_input)
                        adapter_features.append(adapter_output+adapter_input)
                    getattr(self, adapter_id)[-1].load_state_dict(prev_adapter_weight_2)
